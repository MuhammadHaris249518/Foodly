...
def get_embedding(text_to_embed):
    """Generate embedding using a fallback since the current API is not responding."""
    try:
        # For testing, return a zero vector of 1536 size.
        return [0.0] * 1536
...
def ingest_data():
    db = SessionLocal()
    try:
        print(f"Starting ingestion of {len(MOCK_MEALS)} meals...")
        
        # Clear existing meals to avoid duplicates during testing
        db.query(Meal).delete()
        db.commit()
        
        for meal_data in MOCK_MEALS:
